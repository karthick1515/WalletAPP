...
public class TransactionServiceImpl implements TransactionService {
    ...
    public Transaction sendingtransaction(Transaction transaction, Long walletAccountId, int walletPin)
        ...
        if(transaction.getSenderorReceiverid()==walletAccountId) {
        	throw new InvalidAccountIdException("The Receiver ID should not be your Wallet ID");
        }
        ...
        if (transaction.getAmount() < 100 || transaction.getAmount() > 10000000) {
            throw new InvalidAmountException("Check The Entered Amount is valid or Not it Should be between 100 to 10000000");
        }
        if (retrievedSenderWalletAccount.getBalance() < transaction.getAmount() ) {
    ...
    public Transaction addmoneyfrombank(AddMoneyRequest addMoneyRequest)
        ...
        if (!bankAccount.getAccountNumber().equals(retrievedBankAccount.getAccountNumber())
                ...
                ||!bankAccount.getMobileNumber().equals(retrievedBankAccount.getMobileNumber())
                || !bankAccount.getAccountHolderName().equals(retrievedBankAccount.getAccountHolderName())
                || !bankAccount.getAccountType().equals(retrievedBankAccount.getAccountType())
        ...
        if (walletAccount.isEmpty()) {
            throw new InvalidAccountIdException("Wallet Account ID is Wrong , Check Your Profile to Cofirm your Wallet Id");
        }
        
        if (transaction.getAmount() < 100 || transaction.getAmount() > 10000000) {
            throw new InvalidAccountIdException("Check The Entered Amount is valid or Not it Should be between 100 to 10000000");
        }
